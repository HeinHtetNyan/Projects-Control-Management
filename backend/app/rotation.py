...
def rotate_secret_core(db: Session, secret: Secret, new_value: str, actor_name: str) -> Secret:
    ...
    secret.encrypted_value = encrypt_value(new_value.strip())
...
def push_to_single_target(target, secret_name: str, plaintext_value: str) -> dict:
    ...
    try:
        ...
        private_key_pem = decrypt_value(key_secret.encrypted_value)
...
def run_scheduled_rotation_check(db: Session) -> dict:
    """Runs on a schedule from this process's APScheduler job. For 'auto' secrets:
    generate a new value, rotate, and push. For 'reminder' secrets: notify admins
    without changing the value. Commits at the end."""
    now = datetime.utcnow()
    auto_rotated = []
    reminders_sent = []
    errors = []
